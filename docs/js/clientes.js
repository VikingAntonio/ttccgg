...
async function loadClients() {
    $('#client-list').html('<tr><td colspan="5" class="loading">Cargando datos...</td></tr>');
    ...
    if (error) {
        $('#client-list').html('<tr><td colspan="5" class="error">Error al cargar datos.</td></tr>');
        console.error(error);
        return;
    }

    if (!items || items.length === 0) {
        $('#client-list').html('<tr><td colspan="5" class="empty">No tienes clientes registrados.</td></tr>');
        return;
    }
    ...
    items.forEach(item => {
        const $row = $(`
            <tr>
                ...
                <td style="color: #00d2ff; font-weight: bold;">${item.precio_total}</td>
...
async function saveClient() {
    ...
    if (!nombre || !total) {
        Swal.fire('Atención', 'El nombre y el total son obligatorios', 'warning');
        return;
    }
    ...
    let error;
    if (id) {
        const result = await _supabase.from('clientes').update(data).eq('id', id);
        error = result.error;
    } else {
        const result = await _supabase.from('clientes').insert([data]);
        error = result.error;
    }
...
function editClient(item) {
    ...
    $('#input-descuento').val(item.descuento);
...
function resetModal() {
    $('#modal-title').text('Añadir Pedido');
    $('#edit-id').val('');
    $('#input-nombre').val('');
    $('#input-detalles').val('');
    $('#input-total').val('');
...
async function generatePDF(item) {
    ...
    if (item.descuento) {
        $('#tpl-descuento').text(item.descuento);
        $('#tpl-row-descuento').show();
    } else {
        $('#tpl-row-descuento').hide();
    }
    ...
    try {
        ...
        pdf.save(`Pedido_${item.nombre.replace(/\s+/g, '_')}.pdf`);

        Swal.fire('¡Éxito!', 'PDF generado correctamente', 'success');
    } catch (err) {
        console.error(err);
        Swal.fire('Error', 'No se pudo generar el PDF. Verifica que las librerías se cargaron correctamente.', 'error');
    }
}
